...
export function useTradingDataset({
  ...
  return useMemo(() => {
    ...
    const mt5Trades = (report?.trades ?? initialTrades).map((trade) => ({
      ...
      accountId: trade.accountId ?? trade.account_id ?? report?.accountId ?? report?.account_id,
      account_id: trade.account_id ?? trade.accountId ?? report?.account_id ?? report?.accountId,
